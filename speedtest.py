...
try:
    ...
    ofile = open("result/speedtest.json","a")
    tempstdout = sys.stdout
    try:
        #_py3_utf8_stdout = _Py3Utf8Output(sys.stdout)
        #_py3_utf8_stderr = _Py3Utf8Output(sys.stderr)
        _py3_utf8_stdout = _Py3Utf8Output(ofile)
        _py3_utf8_stderr = _Py3Utf8Output(ofile)

    except OSError:
        # sys.stdout/sys.stderr is not a compatible stdout/stderr object
        # just use it and hope things go ok
        #_py3_utf8_stdout = sys.stdout
        _py3_utf8_stdout = ofile
        #_py3_utf8_stderr = sys.stderr
        _py3_utf8_stderr = ofile
    def to_utf8(v):
    ...
    def print_(*args, **kwargs):
        ...
        fp = open("result/speedtest.json","a")
        if fp is None:
            return
...
def build_opener(source_address=None, timeout=30):
...
def parse_args(myargs=[]):
    ...
    #print("parse args ",myargs)
    if len(myargs) > 0:
        options = parser.parse_args(myargs)
        #print("using myargs")
    else:
...
def shell(myargs=[]):
    ...
    #print("shell args ",myargs)
    args = parse_args(myargs = myargs)
...
def main(myargs = []):
    #print(myargs)
    try:
        #if os.path.exists("result/speedtest.txt"):
        #    os.remove("result/speedtest.txt")
        shell(myargs = myargs)
        #print("myshareurl:",shareurl)
